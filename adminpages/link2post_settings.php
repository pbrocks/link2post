...
<div class="wrap">
	...
	<form method="post">
		...
		<p class="description"><?php _e( 'If you want to automatically embed Gists on the frontend using Link2Post, you will also need to install this plugin, oEmbed Gist, by Takayuki Miyauchi: "https://wordpress.org/plugins/oembed-gist/"', 'link2post' ); ?></p>
				...
				<tr>
					...
					<td>
						<select name="l2p_<?php echo($value['quick_name']); ?>_content_enabled">
							...
							<?php
							if ( get_option( 'l2p_' . $value['quick_name'] . '_content_enabled' ) == 'enabled' ) {
								echo('selected="selected"');}
?>
 ><?php _e( 'Yes', 'link2post' ); ?></option>
							<option value="disabled" 
							<?php
							if ( get_option( 'l2p_' . $value['quick_name'] . '_content_enabled' ) == 'disabled' ) {
								echo('selected="selected"');}
?>
 ><?php _e( 'No', 'link2post' ); ?></option>
						</select>
					</td>
					<td>
...
?>
 ><?php _e( 'Yes', 'link2post' ); ?></option>
							<option value="disabled" 
							<?php
							if ( get_option( 'l2p_' . $value['quick_name'] . '_cpt_enabled' ) == 'disabled' ) {
								echo('selected="selected"');}
?>
 ><?php _e( 'No', 'link2post' ); ?></option>
						</select>
