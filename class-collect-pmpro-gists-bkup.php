...
class Collect_PMPro_Gists {
	 ...
	 * @return [type] [description]
	 */
	public function check_for_id_meta() {
		$all_posts = $this->check_posts_for_gist_ids( 'gist' );
		echo '<pre>';
		...
		foreach ( $all_posts as $key => $post ) {
			...
			if ( $meta['gist_id'][0] ) {
				echo "<li>$key " . $post->ID . ' = yes => ' . $meta['gist_id'][0] . '</li>';
			} else {
				echo "<li>$key " . $post->ID . ' = no => ' . $post->post_title . '</li>';
				$gist_url_parsed = $this->parse_gist_url_parts( $meta['l2p_url'][0] );
				update_post_meta( $post->ID, 'gist_id', $gist_url_parsed['id'] );
			}
		}
	}
	...
	public function ajax_returning_function() {
		...
		if ( ! empty( $create_gist ) ) {
			$return['gist_created'] = $create_gist->ID;
		}
		echo '<pre>';
		print_r( $return );
		echo '</pre>';
	...
	public function create_gists_on_website_planning() {
	...
	public function create_admin_menus() {
		add_dashboard_page(
			...
			self::min_caps,
			'dev-page.php',
			array( $this, 'create_gists_on_website_planning' )
		);
	}
